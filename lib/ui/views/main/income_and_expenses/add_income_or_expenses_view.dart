...
class AddIncomeOrExpensesView extends StatelessWidget {
  final bool isIncome;
  const AddIncomeOrExpensesView({Key? key, required this.isIncome})
      : super(key: key);
      ...
      builder: (
        ...
        return StatusBar(
            child: Scaffold(
              ...
              bottomSheet: model.showModelBottomSheet ? Container(
                color: kcNeutral6,
          ...
          appBar: AppBar(
            ...
            title: Text(
              isIncome ? addIncomeText : addExpenseText,
                  ...
                  child: SizedBox(
                    child: Form(
                      ...
                      children: [
                        ...
                        GestureDetector(
                          onTap: model.setShowModelBottomSheet,
                          child:  BuildLabelContainer(
                              label: 'Category', child: Text(
                                  model.category,
                                  style: heading6Style.copyWith(
                                      fontSize: 16,
...
class BuildBottomModelSheet extends StatelessWidget {
  ...
  const BuildBottomModelSheet({ Key? key, required this.isIncome, required this.model }) : super(key: key);
  final radius = const Radius.circular(30);
  @override
  Widget build(BuildContext context) {
    final dataList = isIncome ? model.incomeCategory : model.expensesCategory;
    return Container(
      ...
      child: Column(
        children: [
          Container(
            ...
            child: Row(
              children:[
                Text('${isIncome ? 'Income' : 'Expenses'} Category', style: heading6Style,),
                const Spacer(),
                IconButton(onPressed: model.setShowModelBottomSheet, icon: const Icon(Icons.close))
              ]
            ),
          ),
          Container(
            ...
            child: ListView.separated(
              itemBuilder: (context, index) => GestureDetector(
                ...
                child: Container(
                  ...
                  child: Text(
                    dataList[index],
                    style: heading6Style.copyWith(fontWeight: FontWeight.w400),
                  ),
                ),
              ), separatorBuilder: (context, _) => verticalSpaceVeryTiny, itemCount: dataList.length),
          )
        ],
      ),
    );
